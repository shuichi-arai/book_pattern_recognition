...
#coding:utf-8
"""
LetsTry2-1 (1次元のパーセプトロンの学習規則の実装例)
"""
import numpy as np
...
def show2D( vecW, vecX, X1, X2, ax ):
    ...
    ax.arrow(x=0,y=0, dx=vecW[1], dy=vecW[0], width=0.01, head_width=0.1, head_length=0.2, length_includes_head=True, color='green')
    ax.arrow(x=0,y=0, dx=vecX[1], dy=vecX[0], width=0.01, head_width=0.1, head_length=0.2, length_includes_head=True, color='red')
...
if __name__ == '__main__':
    ...
    plt.savefig('result.LetsTry2-1.png')
    #plt.savefig('result.LetsTry2-1.eps')
    #plt.savefig('result.LetsTry2-1.svg')
    plt.show() # 最後は表示をkeep
