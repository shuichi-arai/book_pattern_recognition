...
def train(vecW, vecX, label, rho):
    ...
    if (np.dot(vecW,vecX) * label < 0):
        ...
        #norm = np.linalg.norm(vecW) #norm = np.sqrt(vecW[0]*vecW[0] + vecW[1]*vecW[1])
        #vecW /= norm
...
def show2D( vecW, vecX, X1, X2, ax ):
    ...
    # 学習用パターン
    ax.scatter(X1[:,1], X1[:,0], c='red' , marker="o") # クラスω1
    ax.scatter(X2[:,1], X2[:,0], c='blue', marker="o") # クラスω2
    ax.legend(('class $\omega_1$', 'class $\omega_2$'))
    ...
    ax.arrow(x=0,y=0, dx=vecW[1], dy=vecW[0], width=0.01, head_width=0.2, head_length=0.4, length_includes_head=True, color='green')
    ax.arrow(x=0,y=0, dx=vecX[1], dy=vecX[0], width=0.01, head_width=0.2, head_length=0.4, length_includes_head=True, color='red')
...
def show( vecW, vecX, X1, X2, sp1, sp2 ):
    """
    2D + 1D 表示
    """

    show2D(vecW, vecX, X1, X2, sp1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-w', '--wvec',  type=tp   , default=[1,3], help='initial W vector: x0,x1')
    parser.add_argument('-s', '--sleep', type=float, default=0.01, help='sleep time between drawing [sec]')
    ...
    for j in range(loop):
        ...
        for vecX, label in zip(matX, labels):
            ...
            if is_mod: # Wが更新された
                ...
                show(vecW, vecX, X1, X2, sp1, sp2) # 表示

        if mod == False:
            print("loop回数", j, ", Wの更新回数", num_update, "で収束しました")
            break

    show(vecW, np.zeros(2), X1, X2, sp1, sp2) # 表示
