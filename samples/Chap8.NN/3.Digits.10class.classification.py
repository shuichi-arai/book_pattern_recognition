...
def disp_digit( img, h, w, no, ans, est ):
    """ 数字の表示 """
...
if __name__ == '__main__':
    ...
    mlp = nn.TwoLayerPerceptron(36, args.num_unit, 10, "tanh", "softmax", args.seed)
    ...
    for loop in range(args.loop): # loop回数
        ...
        print("[%d] cross entropy=%10.7f" % (loop, entropy))
    ...
    no = 0
    for ans, est0, vecX in zip(test_labels, result_infer, test_data):
        est = est0[0]
        if ans != est:
            ...
            sp.axis('off')
            no += 1
            disp_digit(img, 7, 5, no, ans, est) # 端末に表示
    ...
    print("cross entropy = %10.7f" % (mlp.error_Nclass_cross_entropy(g3, test_supervised)))
